...
![Python](https://img.shields.io/badge/Python-3.10+-blue) ![n8n](https://img.shields.io/badge/n8n-Workflow%20Automation-FF6D5A) ![Ollama](https://img.shields.io/badge/Ollama-Local%20Inference-000000) ![Supabase](https://img.shields.io/badge/Supabase-PostgreSQL-3ECF8E)
...
*All results are from synthetic benchmarks. See [Limitations](#️-limitations--caveats) for full context.*
...
1. **Input** – A raw email arrives via webhook.
2. **Extractor Agent** – Parses it into structured JSON (sender, subject, body, URLs, attachments).
3. **Classifier Agent** – Queries Supabase for sender reputation and interaction history; gets a binary verdict from the fine-tuned Llama model (Ollama); passes verdict + grounding context to Claude Haiku for the final 4-class label (phishing / BEC / legitimate / spam).
4. **Validator Agent** – Cross-checks the verdict against rule-based heuristics (domain mismatch, urgency language) and assigns a trust score.
5. **Orchestrator** – Combines all outputs, logs to Supabase, and returns the final JSON response.
